...
const UserProfile: React.FC<UserProfileProps> = ({ onClose, botUsername }) => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!snap.exists()) {
          ...
          // ✅ FIX: Existing user যদি refer link দিয়ে ঢোকে এবং আগে referred না হয়
          if (startParam?.startsWith('ref_') && !data.referredBy) {
            try {
              const refCode = startParam.replace('ref_', '');
              const rq = query(collection(db,'users'), where('referralCode','==',refCode), limit(1));
              const rs = await getDocs(rq);
              if (!rs.empty && rs.docs[0].id !== uid) {
                const referrerId = rs.docs[0].id;
                const sd2 = settingsSnap.exists() ? settingsSnap.data() : {};
                const refBonus = sd2.coinWelcome ?? 50;
                updates.referredBy = referrerId;
                updates.coins = increment(refBonus);
                await addDoc(collection(db,'pendingReferrals'), {
                  referrerId, newUserId: uid, completed: false, createdAt: serverTimestamp(),
                });
                await addCoinHistory(uid, 'earn', `🔗 Referral Join বোনাস`, refBonus);
                showToast(`🎉 Referral Bonus! +${refBonus} Coin 🪙`);
              }
            } catch(re) { console.error('referral error:', re); }
          }
  ...
  // ── Complete referral — App.tsx এ global ভাবে handle হচ্ছে ──────────────
  ...
  // ── Convert ───────────────────────────────────────────────────────────────
  const isConverting = React.useRef(false); // 🔒 Double-click lock
  const handleConvert = async () => {
    if (!tgUser || !userData) return;
    if (isConverting.current) return; // 🔒 Double submit block
    ...
    setTimeout(async () => {
      try {
        const uid = String(tgUser.id);
        // ✅ Server-side verify: Firestore থেকে actual coin check করো
        ...
        if (freshCoins < coins) {
          showToast('Coin কমে গেছে! আবার try করো', 'error');
        ...
        await addCoinHistory(uid, 'spend', `💱 ${coins} Coin → ৳${taka}`, coins);
  ...
  const isWithdrawing = React.useRef(false); // 🔒 Double-submit lock
  const handleWithdraw = async () => {
    ...
    try {
      const uid = String(tgUser.id);
      // ✅ Server-side verify: actual balance check
      const freshSnap = await getDoc(doc(db, 'users', uid));
      if (!freshSnap.exists()) throw new Error('User not found');
      const freshBalance = freshSnap.data().takaBalance || 0;
      if (freshBalance < amount) {
        showToast('Balance কম! আবার check করো', 'error');
        isWithdrawing.current = false;
        setWLoading(false); return;
      }
      // ✅ Bug Fix #3: Pending withdrawal check — একই সময়ে একাধিক request block
      const pendingSnap = await getDocs(
        query(collection(db, 'withdrawals'), where('userId','==',uid), where('status','==','pending'))
      );
      if (!pendingSnap.empty) {
        showToast('আগের request এখনো pending! সেটা complete হলে নতুন request দাও।', 'error');
        isWithdrawing.current = false;
        setWLoading(false); return;
      }
      newDocRef = await addDoc(collection(db, 'withdrawals'), {
        ...
        amount, method: wMethod, number: wNumber,
                    ...
                    <div className="p-4">
                      ...
                      <motion.button
                        ...
                        onClick={async () => {
                          ...
                          // ✅ Bug Fix #2: Reward দেওয়ার আগে Firestore এ server-side verify
                          const uid2 = String(tgUser.id);
                          const { updateDoc, doc:fd, increment:fi, addDoc:fa, collection:fc, serverTimestamp:fs, getDoc:fg, setDoc:fsd } = await import('firebase/firestore');
                          const { db:fdb } = await import('../firebase');

                          // Firestore এ taskProgress check করো
                          const todayStr2 = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
                          const taskProgressRef = fd(fdb, `users/${uid2}/taskProgress`, `${task.id}_${todayStr2}`);
                          const taskProgressSnap = await fg(taskProgressRef);
                          const serverCount = taskProgressSnap.exists() ? (taskProgressSnap.data().count || 0) : 0;

                          if (serverCount >= maxAds) {
                            // Server এ limit পার হয়েছে — localStorage ও sync করো
                            localStorage.setItem(countKey, String(maxAds));
                            setTaskMsg(m=>({...m,[task.id]:'❌ আজকের limit শেষ (server verified)'}));
                            setTimeout(()=>setTaskMsg(m=>({...m,[task.id]:''})),3000);
                            setTaskAdLoading(null);
                            return;
                          }
                          ...
                          try {
                            ...
                            // ✅ Bug Fix #2: Firestore এও save করো (server-side count)
                            if (taskProgressSnap.exists()) {
                              await updateDoc(taskProgressRef, { count: fi(1), lastUpdated: fs() });
                            } else {
                              await fsd(taskProgressRef, { count: 1, taskId: task.id, date: todayStr2, createdAt: fs(), lastUpdated: fs() });
                            }
                            localStorage.setItem(countKey, String(newCount));
                            haptic('success');
                            // ✅ Referral complete trigger
                            if (typeof (window as any).completeCinelixReferral === 'function') {
                              (window as any).completeCinelixReferral(uid2);
                            }
