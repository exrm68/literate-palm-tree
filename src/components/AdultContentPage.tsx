...
// ── Unlock helpers — User ID সহ key, যাতে আলাদা account আলাদা থাকে ──
const getTgUid = () => String((window as any)?.Telegram?.WebApp?.initDataUnsafe?.user?.id || 'guest');
const getPermanentKey = (id: string) => `adult_perm_${getTgUid()}_${id}`;
const getUnlock24Key  = (id: string) => `adult_24_${getTgUid()}_${id}`;
  ...
  const saveUnlock = useCallback(async (permanent: boolean) => {
    ...
    if (tgUser) {
      ...
      try {
        ...
        // ✅ Firestore এ adultUnlockCount বাড়াও — Dashboard এ দেখাবে
        try {
          const userRef = doc(db, 'users', uid);
          const userSnap = await getDoc(userRef);
          if (userSnap.exists()) {
            const unlockedList: string[] = userSnap.data().unlockedAdultIds || [];
            if (!unlockedList.includes(item.id)) {
              await updateDoc(userRef, {
                adultUnlockCount: increment(1),
                unlockedAdultIds: [...unlockedList, item.id],
              });
            }
          }
        } catch {}
