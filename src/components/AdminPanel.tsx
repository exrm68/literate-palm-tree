...
const DashboardPanel: React.FC = () => {
  const [stats, setStats] = useState({
    totalUsers: 0, todayUsers: 0, totalCoins: 0, totalRefers: 0,
    pendingWithdrawals: 0, totalWithdrawn: 0, totalUnlocked: 0, totalAdultUnlocked: 0,
    totalEpisodeUnlocked: 0, // ✅ Bug Fix #5: Episode unlock count
  });
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    ...
    const unsubUsers = onSnapshot(collection(db, 'users'), (snap) => {
      ...
      const todayUsers = allUsers.filter(u => { const j = u.joinedAt?.toDate?.(); return j && j >= today; }).length;
      const totalCoins = allUsers.reduce((s, u) => s + (u.coins || 0), 0);
      const totalRefers = allUsers.reduce((s, u) => s + (u.referralCount || 0), 0);
      const totalUnlocked = allUsers.reduce((s, u) => s + (u.unlockCount || 0), 0);
      const totalAdultUnlocked = allUsers.reduce((s, u) => s + (u.adultUnlockCount || 0), 0);
      // ✅ Bug Fix #5: Episode unlock count
      const totalEpisodeUnlocked = allUsers.reduce((s, u) => s + (u.episodeUnlockCount || 0), 0);
      setStats(prev => ({ ...prev, totalUsers: allUsers.length, todayUsers, totalCoins, totalRefers, totalUnlocked, totalAdultUnlocked, totalEpisodeUnlocked }));
      ...
      // ✅ Top Unlockers sort includes episodeUnlockCount
      const topUnlock = [...allUsers].sort((a, b) => ((b.unlockCount||0)+(b.adultUnlockCount||0)+(b.episodeUnlockCount||0)) - ((a.unlockCount||0)+(a.adultUnlockCount||0)+(a.episodeUnlockCount||0)));
    ...
    const unsubW = onSnapshot(collection(db, 'withdrawals'), (snap) => {
      ...
      setStats(prev => ({ ...prev, pendingWithdrawals: pending, totalWithdrawn }));
    });

    return () => { unsubUsers(); unsubW(); };
  }, []);
  ...
  const statCards = [
    ...
    { label: 'Movie Unlock', value: stats.totalUnlocked.toLocaleString(), icon: '🔓', color: 'text-pink-400', bg: 'bg-pink-500/10 border-pink-500/20' },
    { label: 'Episode Unlock', value: stats.totalEpisodeUnlocked.toLocaleString(), icon: '📺', color: 'text-indigo-400', bg: 'bg-indigo-500/10 border-indigo-500/20' }, // ✅ Bug Fix #5
    { label: '18+ Unlock', value: stats.totalAdultUnlocked.toLocaleString(), icon: '🔞', color: 'text-rose-400', bg: 'bg-rose-500/10 border-rose-500/20' },
  ...
  return (
    <div className="space-y-5 py-2">
      <div className="flex items-center justify-between">
        ...
        {lastUpdated && <span className="text-zinc-500 text-[10px]">🔴 Live · {lastUpdated}</span>}
      </div>
      ...
      <div className="bg-[#1a1a1d] rounded-[20px] p-4 border border-white/5">
        ...
        {topUnlockers.length === 0 ? (
          ...
          <div key={u.id} className={`flex items-center gap-3 py-2.5 ${i < topUnlockers.length - 1 ? 'border-b border-white/5' : ''}`}>
            ...
            <div className="text-right flex-shrink-0">
              <p className="text-pink-400 font-black text-sm">{(u.unlockCount||0)+(u.adultUnlockCount||0)+(u.episodeUnlockCount||0)} total</p>
              <p className="text-zinc-500 text-[9px]">🎬{u.unlockCount||0} 📺{u.episodeUnlockCount||0} 🔞{u.adultUnlockCount||0}</p>
            </div>
          </div>
        ))}
      </div>
