...
import { collection, onSnapshot, doc, query, orderBy, limit, getDocs, startAfter, QueryDocumentSnapshot, DocumentData, where, updateDoc, increment, addDoc, serverTimestamp, getDoc } from 'firebase/firestore';
...
const App: React.FC = () => {
  ...
  // ✅ Global Referral Completion — সবসময় active থাকে, profile tab বন্ধ থাকলেও
  const isCompletingReferral = useRef(false);
  useEffect(() => {
    const completeReferral = async (userId?: string) => {
      const tgId = userId || String((window as any)?.Telegram?.WebApp?.initDataUnsafe?.user?.id || '');
      if (!tgId || isCompletingReferral.current) return;
      isCompletingReferral.current = true;
      try {
        const q = query(
          collection(db, 'pendingReferrals'),
          where('newUserId', '==', tgId),
          where('completed', '==', false),
          limit(1)
        );
        const snap = await getDocs(q);
        if (snap.empty) { isCompletingReferral.current = false; return; }

        const pendingDoc = snap.docs[0];
        const { referrerId, newUserId } = pendingDoc.data();
        if (referrerId === tgId) { isCompletingReferral.current = false; return; }

        // আগে completed হয়েছে কিনা check
        const prevQ = query(
          collection(db, 'pendingReferrals'),
          where('newUserId', '==', tgId),
        ...
        if (!prevSnap.empty) {
          ...
          isCompletingReferral.current = false;
          return;
        }
        ...
        const [referrerSnap, settingsSnap, newUserSnap] = await Promise.all([
          ...
          getDoc(doc(db, 'users', tgId)),
        ]);

        if (!referrerSnap.exists()) { isCompletingReferral.current = false; return; }

        const referrerData = referrerSnap.data();
        const sd = settingsSnap.exists() ? settingsSnap.data() : {};
        const perRefer = sd.coinPerRefer ?? 100;
        const newUserName = newUserSnap.exists() ? (newUserSnap.data().name || 'নতুন বন্ধু') : 'নতুন বন্ধু';

        await updateDoc(doc(db, 'users', referrerId), {
          coins: increment(perRefer),
          referralCount: increment(1),
        });
        await addDoc(collection(db, `users/${referrerId}/coinHistory`), {
          type: 'earn', reason: `👥 Referral — ${newUserName}`, amount: perRefer, createdAt: serverTimestamp(),
        });

        // Milestone bonus
        const newCount = (referrerData.referralCount || 0) + 1;
        const milestones = [
          { count: 5,  bonus: sd.coinMilestone5  ?? 500  },
          { count: 10, bonus: sd.coinMilestone10 ?? 1000 },
          { count: 20, bonus: sd.coinMilestone20 ?? 2000 },
          { count: 50, bonus: sd.coinMilestone50 ?? 5000 },
        ];
        for (const m of milestones) {
          if (newCount >= m.count && !(referrerData.milestonesClaimed || []).includes(m.count)) {
            ...
            await addDoc(collection(db, `users/${referrerId}/coinHistory`), {
              type: 'earn', reason: `🎯 ${m.count} Refer Milestone!`, amount: m.bonus, createdAt: serverTimestamp(),
            });
          }
        }
      } catch (e) {
        console.error('Global completeReferral error:', e);
        isCompletingReferral.current = false;
        return;
      }
      // ✅ Fix: success পরে lock reset করো — পরে আবার call করা যাবে
      isCompletingReferral.current = false;
    };

    // ✅ Window এ register করো — MovieDetails, Tasks, AdultContent সব জায়গা থেকে call করা যাবে
